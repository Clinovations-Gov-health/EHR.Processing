...
async function preprocessCostSharingData(data: Buffer, logger: Debugger) {
    ...
    const dfChunkJsonStrings = dataForge
        .fromCSV(data.toString('utf-8'))
    ...
    const resultChunks = await Promise.all<Record<string, CostSharingPreprocessModel>>(
...
async function preprocessAttributesData(data: Buffer, logger: Debugger) {
    ...
    const resultChunks = await Promise.all<Record<string, PlanAttributePreprocessModel>>(
        dfChunkJsonStrings.map((chunk) => workerPool.queue((worker) => worker.preprocessAttributesData(chunk)))
    );
...
async function preprocessRateData(data: Buffer, logger: Debugger) {
    ...
    const resultChunks = await Promise.all<Record<string, RatePreprocessModel>>(
        dfChunkJsonStrings.map((chunk) => workerPool.queue((worker) => worker.preprocessRateData(chunk)))
    );
    ...
    return resultChunks.reduce(
        (prev, curr) =>
            mergeWith(prev, curr, (obj, src) => {
